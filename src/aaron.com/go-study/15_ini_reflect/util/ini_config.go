...
func UnMarshal(data []byte, result interface{}) (err error) {
	...
	for _, line := range lineArr {
		...
		if line[0] == '[' {
			// 按照大标签去处理
			filedName, err = label(line, typeInfo.Elem())
			if err != nil {
				return
			}
			continue
		...
		// 按照正常数据处理
		err = field(filedName, line, result)
		if err != nil {
			return
		}
	}
...
// 处理大标签
func label(line string, typeInfo reflect.Type) (fieldName string, err error) {
	// 去字符串头和尾
...
// 解析属性
// 参数：大标签名，行数据，对象
func field(fieldName string, line string, result interface{}) (err error) {
